  ...
  return new Promise((resolve, reject) => {
    if (payloadState && payloadState.FaceMatches && payloadState.FaceMatches.length != 0) {
      ...
      const searchedFaceDetails = detectedFaces.filter(x => {
        const detailsToFind = payloadState.SearchedFaceBoundingBox;
        return (
          x.BoundingBox.Top.toFixed(2) === detailsToFind.Top.toFixed(2) &&
          x.BoundingBox.Left.toFixed(2) === detailsToFind.Left.toFixed(2)
        );
      })[0];
